...
- Forecasting framework: `src/ts_contracts.py` (`ForecastOutput`, `EvaluationMetrics`), `src/ts_models.py` (`TSModel` subclasses: SeasonalNaive, SARIMA, ExponentialSmoothing, LightGBM), `src/ts_evaluation.py` (`ModelEvaluator` / `ModelComparison` / `ModelTuner`); diagnostics in `src/ts_plots.py`
- Forecast contracts:
...
- `src/ts_contracts.py` — `ForecastOutput`, `EvaluationMetrics`
- `src/ts_models.py` — `TSModel`, `SeasonalNaiveModel`, `SARIMAModel`, `ExponentialSmoothingModel`, `LightGBMModel`
- `src/ts_evaluation.py` — `ModelEvaluator`, `ResidualDiagnostics`, `ModelComparison`, `RollingOriginEvaluator`, `ModelTuner`
