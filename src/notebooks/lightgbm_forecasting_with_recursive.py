...
def _(mo):
    ...
    15. **Genuine Recursive Forecast** — does `LightGBMModel`'s internal-features
        (recursive) mode change PI coverage, measuring the optimism flagged below instead
        of just naming it
    ...
    **Why a baseline fit comes before tuning/selection**: choosing which features to keep
    ...
    No — recursive forecasting doesn't fix the PI coverage gap, it makes it worse.

Added Section 16 to the notebook: fits LightGBMModel in its internal-features (recursive) mode on the same asset/train/test window as SARIMA, folds it into a 4-way comparison chart. Result on ASSET_001:

Model	PI Coverage
SARIMA	71.4%
LightGBM (external-features, true lags)	50.0%
LightGBM (recursive)	39.6%
The reasoning: recursive compounding does widen point-forecast error with horizon (as expected), but the quantile models are still trained on one-step-ahead residuals — they know nothing about how much error accumulates by step 192. So the interval doesn't widen to match the growing error; the compounding error just lands outside a band that's no wider than before. Verified end-to-end and written up in both the notebook and LIGHTGBM_PLAN.md: the "true lags are optimistic" caveat is real for point-accuracy comparisons, but the coverage gap vs. SARIMA is a separate, genuine calibration issue — fixing it for real would need quantile models trained against multi-step residuals, which the framework doesn't build yet (noted as a Next Steps item).
...
def _(asset_sup, feature_cols, pl):
    ...
    if constant_features:
        print(f"Constant for this asset (correlation undefined): {constant_features}")
...
def _(LightGBMModel, ModelEvaluator, X_train_sel, optuna, y_train):
    ...
    print(
        f"Optuna study: {len(study.trials)} trials, scored on a 2-day validation slice of train"
    )
...
def _(X_test_sel, lgbm, y_test):
    ...
    assert (lower <= yhat).all() and (yhat <= upper).all(), (
        "quantile monotonicity failed"
    )
...
def _(SARIMAModel, pl, test_timestamps, train_timestamps):
    ...
    sarima_train = raw_asset.filter(pl.col("timestamp") <= train_timestamps.max())[
        "metering_kwh"
    ].to_numpy()

    sarima_test = (
        raw_asset.filter(pl.col("timestamp").is_in(pl.Series(test_timestamps)))
        .sort("timestamp")["metering_kwh"]
        .to_numpy()
    )
    ...
    return sarima_forecast, sarima_test, sarima_train


@app.cell
def _(ModelEvaluator, sarima_forecast, sarima_test):
...
def _(mo):
    mo.md(r"""
    **Point-forecast accuracy (MAE/RMSE/MAPE):** LightGBM beats SARIMA by a wide margin in
    both variants — roughly half the error. Tuning + feature selection gives LightGBM a
    further, smaller edge over its own baseline (e.g. RMSE ~0.36 -> ~0.33).

    **PI Coverage tells the opposite story:** SARIMA has the best-calibrated intervals
    (~71%, closest to the 80% target), baseline LightGBM is next (~60%), and the tuned
    LightGBM is actually the worst (~50%) — despite being the most accurate on point
    forecasts. That's the real finding: the tuning objective (Section 7) only minimizes
    RMSE on the validation slice, so it happily narrows the quantile spread in exchange for
    a sharper point forecast, at the cost of calibration.

    **None of the three hit 80% coverage.** All three under-cover — the intervals are
    consistently too narrow for a nominal 80% CI, which is a broader calibration issue, not
    something specific to the tuned model.

    Worth remembering the notebook's own caveat here too: LightGBM's test-set evaluation
    (Sections 4-14) uses true lag values, not recursively forecast ones, so its accuracy
    edge over SARIMA is somewhat optimistic relative to a genuine multi-step deployment.
    Section 16 checks that directly rather than just flagging it.

    Practically: if calibrated uncertainty matters more than point accuracy for this use
    case, the baseline (or an objective that also penalizes miscalibration, not just RMSE)
    is the better choice over the RMSE-tuned model — that's a real tension this chart
    surfaces, not one the notebook resolves.
    """)
    return
...
def _(baseline_metrics, metrics, sarima_metrics):
    ...
    print(
        ...
        "fully apples-to-apples with SARIMA's genuine multi-step forecast — see Section 16."
    )
    return
...
def _(mo):
    mo.md("""
    ## Section 16: Does a Genuine Recursive Forecast Change PI Coverage?

    Sections 4-14 evaluate LightGBM with `X_test` built from true observed lag values — an
    optimistic simplification flagged since the Summary but never measured. This section
    fits `LightGBMModel` in its **internal-features mode** (`X_train=None`, the same mode
    `lightgbm_residual_diagnostics.py` and `ts_model_explorer.py` use): it builds its own
    lag + hour-of-day features from `y_train` alone and forecasts recursively — each step
    beyond the shortest lag feeds prior *predictions* back in as inputs, compounding error
    with horizon the way a real multi-step deployment would. Same asset, same train/test
    split as the SARIMA comparison (`sarima_train`/`sarima_test` from Section 15).
    """)
    return


@app.cell
def _(LightGBMModel, ModelEvaluator, sarima_test, sarima_train):
    lgbm_recursive = LightGBMModel(
        sarima_train, lags=[1, 2, 48, 96], quantile_levels=[0.1, 0.5, 0.9]
    )
    lgbm_recursive.fit()
    recursive_forecast = lgbm_recursive.forecast(
        steps=len(sarima_test), confidence_level=0.80
    )
    recursive_metrics = ModelEvaluator.evaluate(sarima_test, recursive_forecast)

    print(f"Recursive (internal-features) LightGBM: {recursive_metrics}")
    return (recursive_metrics,)


@app.cell
def _(ComparisonPlotter, baseline_metrics, metrics, recursive_metrics, sarima_metrics):
    full_comparison_fig = ComparisonPlotter.metrics_comparison(
        {
            "LightGBM (baseline)": baseline_metrics,
            "LightGBM (tuned+reduced)": metrics,
            "LightGBM (recursive)": recursive_metrics,
...
@app.cell
def _(metrics, recursive_metrics):
    coverage_narrowed = recursive_metrics.pi_coverage < metrics.pi_coverage
    print(
        f"External-features (true lags) PI coverage: {metrics.pi_coverage:.1f}%\n"
        f"Recursive (internal-features) PI coverage: {recursive_metrics.pi_coverage:.1f}%"
    )
    print(
        f"\nRecursive forecasting {'made coverage worse' if coverage_narrowed else 'did not make coverage worse'} "
        "here — recursive compounding widens point-forecast error with horizon, but that "
        "doesn't automatically widen the quantile *interval* to match, since each step's "
        "quantile models were trained on one-step residuals, not on how much recursive "
        "error accumulates by step 192. A coverage gap from true-lag optimism would show up "
        "as recursive coverage being *higher* (closer to 80%) than the external-features "
        "run; a further drop means the compounding error is landing outside even a wider "
        "band, not being caught by it."
    )
    return
...
def _(mo):
    ...
    - Section 16's recursive quantile models are still trained on one-step-ahead pinball
      loss (same as every other `LightGBMModel` fit) — they know nothing about how much
      error a 192-step recursive chain accumulates, so their intervals aren't calibrated
      *for* recursive compounding, just carried through it unchanged. A true fix would need
      quantile models trained against multi-step (not one-step) residuals, which the
      framework doesn't build yet.
